...
public interface FestivalRepository extends JpaRepository<Festival, Long> {
    ...
    //학교
    @Query("select new UniFest.domain.festival.dto.response.FestivalSearchResponse(f.id, s.id, f.thumbnail, s.name, s.region, f.name, f.beginDate, f.endDate, s.latitude, s.longitude)"
    ...
    //오늘
    @Query("select new UniFest.domain.festival.dto.response.TodayFestivalInfo(s.id, s.name, f.thumbnail, f.id, f.name, f.beginDate, f.endDate)"
            + " from School s join Festival f on f.school.id=s.id"
